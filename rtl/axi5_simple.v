...
module tb;
    ...
    wire AWVALID, AWREADY, AWPENDING, AWCRDT;
    wire WVALID,  WREADY,  WPENDING,  WCRDT;
    wire BVALID,  BREADY,  BPENDING,  BCRDT;
    wire ARVALID, ARREADY, ARPENDING, ARCRDT;
    wire RVALID,  RREADY,  RPENDING,  RCRDT;

    axi_manager   mgr (.clk(clk),.rstn(rstn),
        .AWADDR(AWADDR),.AWVALID(AWVALID),.AWREADY(AWREADY),.AWPENDING(AWPENDING),.AWCRDT(AWCRDT),
        .WDATA(WDATA),  .WVALID(WVALID),  .WREADY(WREADY),  .WPENDING(WPENDING),  .WCRDT(WCRDT),
        .BVALID(BVALID),.BREADY(BREADY),  .BCRDT(BCRDT),
        .ARADDR(ARADDR),.ARVALID(ARVALID),.ARREADY(ARREADY),.ARPENDING(ARPENDING),.ARCRDT(ARCRDT),
        .RDATA(RDATA),  .RVALID(RVALID),  .RREADY(RREADY),  .RCRDT(RCRDT));

    axi_subordinate sub (.clk(clk),.rstn(rstn),
        .AWADDR(AWADDR),.AWVALID(AWVALID),.AWREADY(AWREADY),.AWPENDING(AWPENDING),.AWCRDT(AWCRDT),
        .WDATA(WDATA),  .WVALID(WVALID),  .WREADY(WREADY),  .WPENDING(WPENDING),  .WCRDT(WCRDT),
        .BVALID(BVALID),.BREADY(BREADY),  .BPENDING(BPENDING),.BCRDT(BCRDT),
        .ARADDR(ARADDR),.ARVALID(ARVALID),.ARREADY(ARREADY),.ARPENDING(ARPENDING),.ARCRDT(ARCRDT),
        .RDATA(RDATA),  .RVALID(RVALID),  .RREADY(RREADY),  .RPENDING(RPENDING),  .RCRDT(RCRDT));

    // counters
    integer wr=0, rd=0;
    always @(posedge clk) begin
        if (BVALID && BREADY) wr = wr + 1;
        if (RVALID && RREADY) rd = rd + 1;
    end

    initial begin
        $dumpfile("axi5_simple.vcd");
        $dumpvars(0, tb);
        repeat(3) @(posedge clk); rstn = 1;
        repeat(60) @(posedge clk);
        $display("Write txns: %0d  Read txns: %0d", wr, rd);
        if (wr>0 && rd>0) $display("PASS"); else $display("FAIL");
        $finish;
    end
endmodule
